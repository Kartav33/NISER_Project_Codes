...
plt.plot(xaxis,yaxis,color='b', label='$D_3$ using Rk4')
plt.xlabel('\u03BE')
plt.ylabel('$D_n(\u03BE)$')
plt.xlim(0,5)
plt.ylim(0,1.1)
...
Xitpld1=[]
for item in range(1,96):
    Xin=(3*X5n[t]+2*X0n[t])/5   #Linear intepolation equation for n=3
    Xitpld1.append(Xin)
    t=t+1

plt.scatter(Xitpld1,YM,color='red',s=0.7, label='Linear Int.') 
...
for item in Xitpld1:
    ...
    Yr1.append(yrk)
    yer=abs(YM[i]-yrk)
    YP1.append(yer*100/YM[i])
    lerr1.append(yer)
    i=i+1

#plt.scatter(Xitpld1,Yr1,color='b')
plt.errorbar(Xitpld1,Yr1,lerr1, label='error due linear int.',fmt='o')
...
for item in Xitpld:
    ...
    yer=abs(yrk-YM[i])
    lerr2.append(yer)
    YP2.append(yer*100/YM[i])
    i=i+1

#plt.scatter(Xitpld,Yitpld,color='green',s=0.7, label='Qudratic int.')


#plt.errorbar(Xitpld,Yr2,lerr2, label='error due quadratic int.',fmt='g')
...
#print('maximum percentage error due to linear interpolation ', max(YP1))
#print('\nminimum percentage error due to linear interpolation ', min(YP1))
